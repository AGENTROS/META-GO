...
def test_whisper_voice_verifier():
    ...
    transcribed, accuracy, confidence = verifier.transcribe_and_verify(
        audio_bytes, "Verify my sovereign identity"
    )
    # Silence check
    assert (
        "VAD Failed" in transcribed
        or "[No Audio Detected]" in transcribed
        or transcribed == ""
    )
    assert accuracy == 0.0
