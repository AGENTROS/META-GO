...
def test_avatar_validator():
    # Valid asset
    assert (
        AvatarValidator.validate_asset(
            "https://example.com/model.glb",
            {"size_mb": 10, "skeleton_type": "humanoid"},
        )
        is True
    )
    ...
    with pytest.raises(ValueError, match="humanoid"):
        AvatarValidator.validate_asset(
            "https://example.com/model.glb",
            {"size_mb": 10, "skeleton_type": "quadruped"},
        )
...
async def test_avatar_apis():
    async with AsyncClient(
        ...
        payload = {"did": "did:metago:123", "provider": "ready_player_me", "params": {}}
