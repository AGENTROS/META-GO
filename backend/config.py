...
class Config:
    def __init__(self):
        ...
        self.AUTO_MIGRATE_VOICE_TEMPLATES = _parse_bool(os.environ.get("AUTO_MIGRATE_VOICE_TEMPLATES"))
    ...
    def validate_startup(self):
        ...
        if self.is_production() and self.TEST_MODE:
            raise RuntimeError("MetaGo startup blocked: TEST_MODE cannot be enabled in production.")
        ...
        if self.is_production() and not self.MONGO_URL:
            raise RuntimeError("CRITICAL CONFIGURATION ERROR: MONGO_URL must be set in production.")
    ...
    def require_voice_key_for_production(self):
        if self.is_production() and not self.VOICE_TEMPLATE_KEY:
            raise RuntimeError("CRITICAL CONFIGURATION ERROR: VOICE_TEMPLATE_KEY must be set in production.")
