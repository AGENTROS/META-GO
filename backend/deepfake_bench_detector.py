    ...
    def predict_risk(self, face_image: np.ndarray, landmarks: list = None) -> float:
        ...
        if self.has_model:
            try:
                ...
                print(f"DeepfakeBench neural prediction error: {e}. Falling back to analytical mode.")

        return self._run_analytical_fallback(face_image, landmarks)

    def _run_analytical_fallback(self, img: np.ndarray, landmarks: list = None) -> float:
        """
        ...
        try:
            ...
            boundary_sat_mean = np.mean(s_chan[boundary_mask == 255]) if np.sum(boundary_mask == 255) > 0 else 0
            inner_sat_mean = np.mean(s_chan[inner_mask == 255]) if np.sum(inner_mask == 255) > 0 else 0
            ...
            if landmarks and len(landmarks) >= 5:
                try:
                    ...
                    nose_to_mouth = np.linalg.norm(np.array(nose) - (np.array(ml) + np.array(mr))/2.0)
