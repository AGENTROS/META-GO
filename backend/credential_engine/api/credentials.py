...
from credential_engine.models.credential import (
    CredentialIssuanceRequest,
    IssuedCredential,
)
from credential_engine.repository.db import (
...
async def upload_document(
    file: UploadFile = File(...), document_type: str = Form("Aadhaar")
):
    """
    ...
    EventPublisher.publish(
        "Credential.Uploaded", {"filename": file.filename, "type": document_type}
    )
    ...
    try:
        ...
        EventPublisher.publish(
            ...
            {"type": document_type, "confidence": ocr_result.overall_confidence},
        )
...
async def issue_credential(request: CredentialIssuanceRequest):
    ...
    # 4. Fire Events
    EventPublisher.publish(
        "Credential.Verified", {"id": credential.id, "owner": credential.owner_wallet}
    )
    EventPublisher.publish(
        "Credential.Issued", {"id": credential.id, "type": credential.document_type}
    )
