...
async def create_burner_did(request: Request, address: str, body: CreateBurnerRequest):
    ...
    await log_privacy_event(
        db, address, "Burner DID Created", {"duration_hours": body.duration_hours}
    )
    return {
        "success": True,
...
async def revoke_burner_did(request: Request, address: str, burner_id: str):
    ...
    try:
        ...
        await log_privacy_event(
            db, address, "Burner DID Revoked", {"burner_id": burner_id}
        )
        return {"success": True}
    except Exception:
...
async def delete_vault_item(request: Request, item_id: str, address: str):
    ...
    try:
        res = await db.vault_items.delete_one(
            {"_id": ObjectId(item_id), "owner_wallet": address.lower()}
        )
...
async def share_credential(
    request: Request, address: str, body: ShareCredentialRequest
):
    db, _ = await get_secure_context(request, address)
    ...
    await log_privacy_event(
        db, address, "Credential Shared", {"receiver": body.receiver}
    )
    return {"success": True}
...
async def get_context_risk(request: Request, address: str):
    ...
    sess_cursor = (
        db.sessions.find({"walletAddress": address.lower()})
    ...
    audit_cursor = (
        db.audit_logs.find({"walletAddress": address.lower(), "risk": "High"})
        .sort("time", -1)
        .limit(1)
    )
    high_risks = (
        [doc async for doc in audit_cursor]
        if hasattr(audit_cursor, "__aiter__")
        else []
    )
    if high_risks:
...
async def get_time_machine(request: Request, address: str):
    ...
    priv_c = (
        db.privacy_events.find({"walletAddress": address.lower()})
        .sort("time", -1)
        .limit(50)
    )
    ...
    if hasattr(audit_c, "__aiter__"):
        events.extend(
            [
                {"type": "audit", "event": d.get("event"), "time": d.get("time")}
                async for d in audit_c
            ]
        )
    if hasattr(priv_c, "__aiter__"):
        events.extend(
            [
                {"type": "privacy", "event": d.get("event"), "time": d.get("time")}
                async for d in priv_c
            ]
        )
        ...
        events = [
            {
                ...
                "type": "privacy",
                "event": "Zero-Knowledge KYC Proof Generated for Binance",
            },
            {
                "time": (now - timedelta(hours=2)).isoformat() + "Z",
                "type": "audit",
                "event": "Smart Contract Interaction: Uniswap V3 Router",
            },
            {
                "time": (now - timedelta(hours=24)).isoformat() + "Z",
                "type": "privacy",
                "event": "Burner DID #492 rotated to preserve anonymity",
            },
            {
                "time": (now - timedelta(hours=48)).isoformat() + "Z",
                "type": "audit",
                "event": "Wallet Connect Session Initiated: OpenSea",
            },
            {
                "time": (now - timedelta(hours=72)).isoformat() + "Z",
                "type": "privacy",
                "event": "Selective Disclosure: Shared 'Age > 18' with Coinbase",
            },
        ]
