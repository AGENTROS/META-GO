...
class RuleEngine:
    ...
    def calculate_humanity(user: Dict, zks: List, sbts: List) -> float:
        score = 0.0
        if user.get("face_verified"):
            score += 40.0
        if user.get("voice_verified"):
            score += 30.0
        if len(zks) > 0:
            score += 15.0
        if len(sbts) > 0:
    ...
    def explain_trust(context: Dict) -> str:
        ...
        if user.get("face_verified") and user.get("voice_verified"):
            positives.append(
                "Identity is fully verified with face and voice biometrics."
            )
        ...
        if len(context["credentials"]) > 0:
            positives.append(
                f"Holding {len(context['credentials'])} verified credentials."
            )
        else:
        ...
        if high_risk:
            negatives.append(
                f"{len(high_risk)} recent security alert(s) negatively impacting your score."
            )
        ...
        if negatives:
            explanation += (
                "Areas for improvement include: " + ", ".join(negatives) + ". "
            )
...
async def build_context(db, address: str) -> Dict:
    ...
    log_cursor = (
        db.audit_logs.find({"walletAddress": address.lower()})
        .sort("time", -1)
        .limit(10)
    )
...
async def ask_ai_guardian(request: Request, address: str, body: AskRequest):
    ...
    if any(
        greet in query
        for greet in ["hi", "hello", "hey", "who are you", "what can you do", "help"]
    ):
        response_text = (
    ...
    # 2. Trust Score & recommendations
        ...
        keyword in query
        for keyword in ["trust", "score", "explain", "recommend", "what can you do"]
    ):
        explanation = RuleEngine.explain_trust(context)
        recs = RuleEngine.get_recommendations(context)
        recs_str = "\n".join(
            [f"- **{r['action']}** (Impact: {r['impact']})" for r in recs]
        )
        ...
        if recs:
            ...
            response_text += (
                "Great job! You have no outstanding security recommendations."
            )

        structured_data = {"score": context["trust_score"], "recommendations": recs}

    # 3. Humanity score & Biometric verification details
    elif any(
        keyword in query
        for keyword in ["humanity", "biometric", "face", "voice", "identity state"]
    ):
        user = context["user"]
    ...
    # 4. Security status, active alerts and audit logs
        ...
        keyword in query
        for keyword in ["security", "alert", "threat", "session", "log"]
    ):
        high_risk = [l for l in context["audit_logs"] if l.get("risk") == "High"]
        recent_events = "\n".join(
            [
                f"- {l.get('event', 'Audit event')} ({l.get('time', 'unknown')}) [Risk: {l.get('risk', 'Low')}]"
                for l in context["audit_logs"][:3]
            ]
        )
    ...
    # 5. Sandboxed / Rejected prompts (dimag and restriction)
        ...
        structured_data = {"restricted": True}
    ...
    await db.guardian_conversations.update_one(
        ...
        {
            "$push": {"messages": {"$each": [memory_doc], "$slice": -10}},
            "$set": {"updated_at": datetime.now(timezone.utc)},
        },
        upsert=True,
    )

    return {"reply": response_text, "context_snapshot": structured_data}


@router.post("/api/dashboard/intelligence/transcribe")
async def transcribe_audio(
    request: Request, address: str = Form(...), audio: UploadFile = File(...)
):
...
async def get_trust_analytics(request: Request, address: str):
    ...
    timeline = [
        {
            "month": "Jan",
            "trust": max(0, current_trust - 25),
            "humanity": max(0, current_humanity - 40),
        },
        {
            "month": "Feb",
            "trust": max(0, current_trust - 15),
            "humanity": max(0, current_humanity - 20),
        },
        {
            "month": "Mar",
            "trust": max(0, current_trust - 5),
            "humanity": max(0, current_humanity - 10),
        },
