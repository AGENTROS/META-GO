...
class AvatarManager:
    ...
    async def create_avatar(
        self, did: str, provider_name: str, params: dict
    ) -> AvatarModel:
        ...
        await self.event_bus.publish(
            "Avatar.Created",
            {"avatar_id": avatar.avatar_id, "did": did, "provider": provider_name},
        )
        return avatar

    async def import_avatar(
        self, did: str, provider_name: str, asset_url: str, metadata: dict
    ) -> AvatarModel:
        ...
        await self.event_bus.publish(
            "Avatar.Imported",
            {"avatar_id": avatar.avatar_id, "did": did, "asset_url": asset_url},
        )
        return avatar
    ...
    async def delete_avatar(self, avatar_id: str) -> None:
        ...
        if avatar:
            ...
            await self.event_bus.publish(
                "Avatar.Deleted", {"avatar_id": avatar_id, "did": avatar.did}
            )
