...
class ZKProvider(ABC):
    ...
    @abstractmethod
    async def verify_commitment(self, commitment: str, data: bytes, secret: bytes) -> bool:
        """Verify that the raw data and secret match the commitment."""
        pass

    @abstractmethod
    async def generate_proof(self, commitment: str, private_inputs: dict) -> Dict[str, Any]:
