...
class EventDispatcher:
    ...
    async def dispatch(self, event: MetaGoEvent):
        ...
        tasks = [asyncio.create_task(self._safe_execute(cb, event)) for cb in all_callbacks]
    ...
    async def _safe_execute(self, callback: EventCallback, event: MetaGoEvent):
        try:
            ...
            logger.error(f"Error in event handler for {event.event}: {str(e)}", exc_info=True)
