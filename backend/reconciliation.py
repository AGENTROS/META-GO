...
async def verify_transaction_confirmations(
    w3: Web3, tx_hash_str: str
        ...
        try:
            ...
            async for sbt in sbt_cursor:
                ...
                if status == "confirmed":
                    ...
                    logger.info(
                        f"SBT transaction {tx_hash} confirmed with {REQUIRED_CONFIRMATIONS}+ blocks."
                    )
                elif status == "failed":
                    await db.sbts.update_one(
                        {"_id": sbt["_id"]},
                    ...
                    logger.warning(
                        f"SBT transaction {tx_hash} failed on-chain. Marking as FAILED."
                    )
                    ...
                    try:
                        ...
                        if age > 300:  # 5 minutes
                            ...
                            logger.warning(
                                f"SBT transaction {tx_hash} not found on-chain after 5 mins. Marked as DROPPED."
                            )
                    except Exception:
                        pass
            ...
            async for sync in sync_cursor:
                ...
                if status == "confirmed":
                    ...
                    logger.warning(
                        f"Cross-chain sync transaction {tx_hash} failed on-chain."
                    )
                ...
                if time.time() - created_at > 60:  # stuck for > 60 seconds
                    await db.sync_operations.update_one(
                        {"_id": op["_id"]},
                        {
                            "$set": {
                                "status": "failed",
                                "error": "Operation timeout/stuck resolved by sweeper",
                                "failedAt": _now_iso(),
                            }
                        },
                    )
                    logger.warning(
                        f"Stuck idempotency lock resolved for operationId {op.get('operationId')}"
                    )
    ...
    while _running:
        try:
            ...
            if last_processed_block < current_block:
                ...
                logger.info(
                    f"Scanning blockchain events from block {last_processed_block} to {to_block}"
                )
                ...
                if sbt_addr:
                    ...
                    mint_filter = sbt_contract.events.Minted.create_filter(
                        from_block=last_processed_block, to_block=to_block
                    )
                    ...
                    for event in events:
                        ...
                        if not existing:
                            ...
                            logger.info(
                                f"Backfilled missing SBT #{token_id} for {to_addr} from blockchain events."
                            )
...
async def nightly_reconciliation(db):
    ...
    while _running:
        ...
        try:
            ...
            if not relayer.available():
                logger.warning(
                    "Reconciliation skipped: blockchain relayer is not connected."
                )
            ...
            registry_contract = w3.eth.contract(
                address=Web3.to_checksum_address(registry_addr), abi=identities_abi
            )
            sbt_contract = w3.eth.contract(
                address=Web3.to_checksum_address(sbt_addr), abi=SBT_ABI
            )
                ...
                try:
                    ...
                    if active:
                        ...
                        if not user.get("did"):
                            ...
                            logger.warning(
                                f"Reconciled missing DID for {wallet} (Restored from on-chain identity)."
                            )
                    else:
                        # If user is active in DB but active is false on-chain, execute database rollback
                        logger.error(
                            f"[RECONCILIATION ROLLBACK] User {wallet} is active in DB but inactive on-chain. Deleting database records."
                        )
                        try:
                        ...
                        await db.zk_proofs.delete_many(
            ...
            async for sbt in sbt_cursor:
                ...
                try:
                    ...
                    if balance == 0:
                        logger.error(
                            f"CONSISTENCY WARNING: Wallet {wallet} has SBT marked VALID in DB, but 0 balance on-chain!"
                        )
                        ...
                        await db.sbts.update_one(
                            ...
                            {
                                "$set": {
                                    "status": "ORPHANED_ONCHAIN_MISSING",
                                    "reconciledAt": _now_iso(),
                                }
                            },
                        )
