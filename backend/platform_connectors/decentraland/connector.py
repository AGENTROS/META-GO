...
class DecentralandConnector(PlatformConnector):
    ...
    async def authenticate(self, payload: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            await self.transport.send({"event": "Sync.Identity", "version": "1.0", "payload": {"did": f"did:metago:{self.wallet_address}", "name": avatar_name, "status": "synchronized"}})
            await self.transport.send({"event": "Sync.Avatar", "version": "1.0", "payload": {"model": f"dcl_avatar_{len(avatar_wearables)}_items", "status": "synchronized"}})
            await self.transport.send({"event": "Sync.Presence", "version": "1.0", "payload": {"status": "online"}})
            await self.transport.send({"event": "Sync.Credentials", "version": "1.0", "payload": {"count": len(avatar_wearables), "status": "synchronized"}})
            ...
            await manager.broadcast_event(self.wallet_address.lower(), "notification", {
