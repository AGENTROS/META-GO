...
export default function BillingPageClient({ initialPlans }: Props) {
  ...
  useEffect(() => {
    ...
    // Fetch fresh plans in background
    fetch(`${backend}/api/billing/plans`)
      .then(r => r.json())
      .then(d => { if (d.plans?.length) setPlans(d.plans); })
      .catch(() => {});

    // Fetch user's active subscription if wallet connected
    if (address) {
      fetch(`${backend}/api/billing/subscription/${address}`)
        .then(r => r.json())
    ...
    <main className="pt-24 pb-20 px-4 max-w-7xl mx-auto">
      ...
      <div className="text-center mb-12">
        <h1 className="text-4xl md:text-5xl font-extrabold tracking-tight mb-3">
          Identity <span className="gradient-text font-bold">as a Service</span>
        </h1>
