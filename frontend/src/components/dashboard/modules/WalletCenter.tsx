    ...
    async function fetchData() {
      try {
        const [overviewRes, txRes, contractRes] = await Promise.all([
          fetch(`http://localhost:8001/api/wallet/${dummyAddress}`),
          fetch(`http://localhost:8001/api/wallet/history?address=${dummyAddress}`),
          fetch(`http://localhost:8001/api/wallet/permissions?address=${dummyAddress}`)
        ]);
